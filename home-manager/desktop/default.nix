{
  ...
  programs.rofi = {
    enable = true;
    # `$mainMod+D` -> `rofi -show drun` (hypr.conf). The rasi below is built
    # entirely from the active palette, so the launcher tracks whatever theme
    # themes/default.nix selects. It used to be gated on rosepine, with every
    # other theme falling back to a stock rofi theme name that matched nothing.
    theme = let
