...
in {
  ...
  contrastColor = bgHex: lightHex: darkHex: let
    rgb = hexToRgb bgHex;
    ...
    then knownColor
      ...
      toColorLevel = val: let
        ...
        findIndex = lst: target: idx:
          if idx >= builtins.length lst
          then 0
          else if builtins.elemAt lst idx == target
          then idx
