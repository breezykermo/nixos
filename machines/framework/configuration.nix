{ config, lib, pkgs, userName, ... }:
{
  ...
  # ── Framework DESKTOP (Ryzen AI MAX+ 395 / Strix Halo) as an always-on server ──
  # This machine (localProfile "homework") has no battery and no lid, and is
  # SSH'd into for long-running tasks (see docs/remote-ssh.md). The shared laptop
  # power module (machines/modules/power-management.nix) is wrong here: TLP was
  # pinning the powersave governor AND disabling turbo boost (CPU_BOOST_ON_AC = 0),
  # capping this 16-core/32-thread chip to base clock. thermald is Intel-only and
  # does nothing on AMD. Disable the laptop tooling and tune for desktop throughput.
